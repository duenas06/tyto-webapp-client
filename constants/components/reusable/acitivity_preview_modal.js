...
export default function ActivityPreviewModal({
  ...
  function pushRoute() {
    ...
    localStorage.setItem(localStorageRoomData, JSON.stringify(val));
  }
    ...
    <Modal closeOnOverlayClick={false} isOpen={isOpen} onClose={onClose}>
      ...
      <ModalContent>
        ...
        <ModalBody pb={6}>
          ...
          <Divider my={"5"} />
          <Heading fontSize={"md"}>{`${type} Items`}</Heading>
          <Text>{`Below is the preview of the ${type.toLowerCase()} item${
            type !== "Recitation" ? "s" : ""
          }`}</Text>
          <VStack alignItems={"stretch"}>
            {val.items &&
              val?.items.map((data, index) => {
                return (
                  <>
